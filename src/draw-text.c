...
void apply_alpha (SDL_Surface *text, Uint8 alpha) {
	// new_alpha = pixel * alpha >> 8
	Uint32 Amask, Ashift;
	int g, h;
	Uint32 *row;
	Uint32 pixel;
	...
	Amask = text->format->Amask;
	Ashift = text->format->Ashift;
	
	if (Amask == 0) return;
	
	for (g = 0; g < text->h; g++) {
		row = (Uint32*) (text->pixels + (text->pitch * g));
		for (h = 0; h < text->w; h++) {
			pixel = row[h];
